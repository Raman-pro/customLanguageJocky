// JOCKY build 7AEB53A25D2E569D seed=-2 date=2026-09-03 02:07:11
#include <stdio.h>
#include <stdint.h>
#include <stdbool.h>
#include <string.h>
...
static volatile int32_t j_ez58z4fr = 19429;
static volatile int32_t j_krm8vp4x = 11140;
...
static int32_t sum_up_to(int32_t limit)
{
    int32_t total = 0;
    for (int32_t j = 0; (j < limit); j = (j + 1))
    {
        total = (total + j);
    }
    return total;
...
int32_t main(void)
{
    ...
    int32_t answer = sum_up_to(10);
    (void)printf("%s\n", greeting);
    (void)printf("%d\n", answer);
    const char* s = "prime";
    if ((is_prime(7) && (j_str_len(s) == 5)))
    {
        (void)printf("%s\n", true ? "true" : "false");
    }
    else
