#!/usr/bin/env bash
# Read/jocky (LLVM backend) test runner: compiles each *.rd in scripts/
# against expected output stored next to it as *.expect, plus a few
...
for bad in scripts/errors_*.rd; do
    [ -f "$bad" ] || continue
