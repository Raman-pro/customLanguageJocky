// Read / JOCKY forensic runtime.
// Implements the standard-library collectors the compiler calls as external
// functions (j_sys_*, j_env_*, j_net_*, j_fs_*, j_reg_*, j_mem_*, j_str_*).
// Cross-platform: Linux, macOS, Windows via #ifdef. Compiled by the driver at
// --build time and linked against the LLVM-generated object.
#include <stdbool.h>
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
...
int32_t j_str_len(const char* s) { return (int32_t)strlen(s); }
bool j_str_eq(const char* a, const char* b) { return strcmp(a, b) == 0; }
...
const char* j_env_get(const char* name) {
...
void j_net_post(const char* label, const char* value) {
...
const char* j_sys_process_list(void) {
    size_t off = 0;
    j_buf[off++] = '[';
    int first = 1;
#if defined(_WIN32)
...
bool j_fs_exists(const char* path) {
#if defined(_WIN32)
...
const char* j_fs_read(const char* path) {
...
void j_fs_write(const char* path, const char* data) {
    FILE* f = fopen(path, "wb");
    if (f) { fputs(data, f); fclose(f); }
}

const char* j_fs_list(const char* dir) {
    size_t off = 0;
    j_buf[off++] = '[';
    int first = 1;
#if defined(_WIN32)
...
// ---- registry (Windows only; degrades to []) -----------------------------------

const char* j_reg_list(const char* key) {
    size_t off = 0;
    j_buf[off++] = '[';
    int first = 1;
#if defined(_WIN32)
...
// ---- memory dump (Windows only; no-op elsewhere) -------------------------------

void j_mem_dump(int32_t pid, const char* path) {
#if defined(_WIN32)
