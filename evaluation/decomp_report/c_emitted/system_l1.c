...
int32_t main(void)
{
    ...
    (void)printf("%s\n", "== env.get() ==");
    (void)printf("%s\n", j_env_get("USER"));
    (void)printf("%s\n", j_env_get("NOT_A_REAL_VAR"));
    (void)printf("%s\n", "== sys.sleep() ==");
    (void)j_sys_sleep(200);
    (void)printf("%s\n", "slept 200ms");
    static volatile int64_t j_ghb9foiw = 86933;
    if ((((j_ghb9foiw * (j_ghb9foiw + 1)) % 2) == 1))
    {
        (void)printf("%s\n", 0 ? "true" : "false");
    }
    (void)printf("%s\n", "== sys.process_list() ==");
    ...
    (void)printf("%s\n", "== net.sockets() ==");
    (void)printf("%s\n", j_net_sockets());
    static volatile int64_t j_o6t0nvls = 81491;
    if ((((j_o6t0nvls * j_o6t0nvls) + j_o6t0nvls) < 0))
    {
        volatile int64_t j_2wrj8m2p = ((j_o6t0nvls * 27) + 491);
        (void)printf("%s\n", 0 ? "true" : "false");
    }
    (void)printf("%s\n", "== fs.write / fs.read ==");
    (void)j_fs_write("/tmp/jocky_system_tour.txt", "hello from system tour\n");
    (void)printf("%s\n", j_fs_read("/tmp/jocky_system_tour.txt"));
    (void)printf("%s\n", "== fs.exists ==");
    (void)printf("%s\n", j_fs_exists("/tmp/jocky_system_tour.txt") ? "true" : "false");
    (void)printf("%s\n", j_fs_exists("/no/such/path") ? "true" : "false");
    (void)printf("%s\n", "== fs.list ==");
    (void)printf("%s\n", j_fs_list("/tmp"));
    (void)printf("%s\n", "== reg.list (Windows only) ==");
    (void)printf("%s\n", j_reg_list("HKLM\\SOFTWARE\\Microsoft"));
    (void)printf("%s\n", "== mem.dump (Windows only) ==");
    (void)j_mem_dump(1, "/tmp/jocky_mem.bin");
    (void)printf("%s\n", "done");
    return 0;
}
